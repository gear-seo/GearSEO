<?php
/*
Plugin Name: Ocultar Notificações
Description: Este plugin oculta todos os avisos de todos os temas e plugins que ficam poluindo o seu painel do wp-admin
Version: 10.0
Plugin URI: http://www.dasak.com.br
Author: Agencia Dasak
Author URI: http://www.dasak.com.br
TextDomain: dasak
*/

/*  Copyright 2018  Dasak  (email : agencia@dasak.com.br)
...
class WP_HPUW
{
	...
	public function load_menu() 
	{
		...
		{
			add_menu_page(__("Ocultar Notificações","wphpuw"),__("Ocultar Notificações","wphpuw"),"manage_options","wphpuw",array($this,'settingsForm'),"dashicons-visibility");
		}
	}
	...
	public function settingsForm()
	{
		...
		$page .= '<h2>'.__("Ocultar Notificações","wphpuw").'</h2>';
		$page .= '</div>';
		$page .= '<div class="mlw-box-left">';
		$page .= '<a href="http://www.dasak.com.br" class="mlw-logo"><img src="'.plugin_dir_url(__FILE__).'images/logo.png"></a>';
		$page .= '<p class="mlw-copyright">Desenvolvido por&nbsp;<a href="http://www.dasak.com.br">Dasak</a></p>';
		$page .= '<h4>'.__("Conheça nossas soluções web. Diversos plugins e temas que te ajudarão a alcançar um excelente posicionamento na web e ter laytout incríveis com alto desempenho e boas notas no Google","wphpuw").'</h4>';
		$page .= 'Acesse: <a href="https://loja.dasak.com.br" target="_blank"><font size="5">Loja Virtual</font></a>';
		$page .= '</div>';
		...
		$page .= '<div class="mlw-hide-warning"><input type="checkbox" name="options[notifications]" value="1"'.(($this->options["notifications"]==1)?' checked="checked"':'').' id="wphpuw_notifications" style="display:none;" /><h4>'.__("Deixe (ON) para ocultar notificações e (OFF) para exibir notificações DE TUDO","wphpuw").'&nbsp;<div class="wphpuw_checkbox'.(($this->options["notifications"]==1)?' checked':'').'" onclick="cbClick(this,\'wphpuw_notifications\')"></div></h4></div>';
		$page .= '</div>';
		$page .= $this->displayActivePluginsList();
		$page .= '</div>';
		$page .= '<div class="mlw-clear"></div>';
		$page .= '<div class="mlw-footer"><input type="submit" value="'.__("SALVAR CONFIGURAÇÕES","wphpuw").'" class="button button-primary button-large" /></div>';
		$page .= '</form>';
		$page .= '</div>';
		// $page="<style>h4{margin-bottom:5px;}</style>";
		// $page.='<h2>'.__("Configuração das notificações","wphpuw").'</h2>';
	...
	<div class="wrap">
		<h3><?php _e("Selecione os plugins que deseja ATIVAR as notificações","wphpuw");?><p id="mlw-right-button" onclick="cbAll(jQuery(this))" class=""></p></h3>
